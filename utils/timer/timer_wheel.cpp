...
void TimerWheel::Schedule(std::uint32_t pos, std::uint32_t extra, std::uint64_t timeout)
{
    // Sanity checks
    if(pos >= cap_)
        GetLogger().Fatal("[TimerWheel]: 'Schedule' expected 'pos' to be less than wheel capacity, got: ", pos);
...
{
    if(pos >= cap_)
        GetLogger().Fatal("[TimerWheel]: 'Cancel' expected 'pos' to be less than wheel capacity, got: ", pos);
...
void TimerWheel::Unlink(std::uint32_t pos)
{
    // Sanity checks
    if(pos >= cap_)
        GetLogger().Fatal("[TimerWheel]: 'Unlink' expected 'pos' to be less than wheel capacity, got: ", pos);
