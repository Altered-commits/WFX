...
enum class PerIoOperationType { INVALID = -1, ARM_RECV, RECV, SEND, SEND_FILE, ACCEPT, ACCEPT_DEFERRED };
 ...
 * PostAcceptOp -> For the operation performed after Accepting the connection (setting socket options and calling
 * acceptCallback and stuff)
 */
struct PostRecvOp : PerIoBase {
    PostRecvOp()
    {
        operationType = PerIoOperationType::ARM_RECV;
    }
};
