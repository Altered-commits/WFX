...
static inline const char* HttpStatusToReason(HttpStatus code)
{
    switch(code) {
        // 1xx Informational
        ...
        case HttpStatus::NON_AUTHORITATIVE_INFO:           return "Non-Authoritative Information";
...
static inline HttpMethod HttpMethodToEnum(StringView method)
{
    if(method.Equals(SV_GET))  return HttpMethod::GET;
    if(method.Equals(SV_POST)) return HttpMethod::POST;
...
{
    if(version.Equals(SV_HTTP11)) return HttpVersion::HTTP_1_1;
    if(version.Equals(SV_HTTP10)) return HttpVersion::HTTP_1_0;
    if(version.Equals(SV_HTTP20)) return HttpVersion::HTTP_2_0;
