...
RouteSegment::RouteSegment(std::string_view key, std::unique_ptr<TrieNode> c)
    : routeValue(SegmentVariant::FromString(StringView{key.data(), key.size()}, true)), child(std::move(c))
{}

RouteSegment::RouteSegment(SegmentVariant p, std::unique_ptr<TrieNode> c) : routeValue(p), child(std::move(c))
{}
...
bool RouteSegment::MatchesStatic(std::string_view candidate) const
{
    ...
    return sv.Size() == candidate.size() && std::memcmp(sv.data, candidate.data(), sv.Size()) == 0;
}

ParamType RouteSegment::GetParamType() const
{
    switch(routeValue.Tag()) {
        case SEG_VARIANT_U64:
            return ParamType::UINT;
        case SEG_VARIANT_I64:
            return ParamType::INT;
        case SEG_VARIANT_STR:
            return ParamType::STRING;
        case SEG_VARIANT_UUID:
