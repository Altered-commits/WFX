...
template <typename T1, typename T2> bool CaseInsensitiveEqual::operator()(const T1& lhs, const T2& rhs) const
{
    static_assert((std::is_same_v<std::decay_t<T1>, std::string> ||
                   std::is_same_v<std::decay_t<T1>, std::string_view>) &&
                      (std::is_same_v<std::decay_t<T2>, std::string> ||
                       std::is_same_v<std::decay_t<T2>, std::string_view>),
                  "CaseInsensitiveEqual: both operands must be std::string or std::string_view");
    return Utils::StringUtils::CTInsensitiveStringCompare(lhs, rhs);
}

template <typename K, typename V> HttpHeaders<K, V>::HttpHeaders()
{
...
template <typename K, typename V> void HttpHeaders<K, V>::SetHeader(K key, V value)
{
...
template <typename K, typename V> V HttpHeaders<K, V>::GetHeader(const K& key) const
{
...
typename HttpHeaders<K, V>::HeaderResult HttpHeaders<K, V>::CheckAndGetHeader(const K& key) const
{
    auto it = headers_.find(key);
    if(it != headers_.end())
...
template <typename K, typename V> void HttpHeaders<K, V>::RemoveHeader(const K& key)
{
    headers_.erase(key);
}

template <typename K, typename V> void HttpHeaders<K, V>::Clear()
{
    headers_.clear();
}

template <typename K, typename V> typename HttpHeaders<K, V>::HeaderMapType& HttpHeaders<K, V>::GetHeaderMap()
{
    return headers_;
}
