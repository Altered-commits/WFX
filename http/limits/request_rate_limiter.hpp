...
// never erased just because refCount drops to zero: Acquire()/Release() only adjust refCount, and
// an entry only disappears via LRU eviction once the tracked-identity cap is hit, which itself
// skips any entry still tied to a live connection (refCount > 0).
...
private:
    ...
    // non-live entry if the pool is already full). 'wasCreated' distinguishes a brand-new entry
    // (bucket needs seeding) from an existing one (bucket must be left alone). Returns nullptr
    // only when every tracked identity currently has a live connection on it.
