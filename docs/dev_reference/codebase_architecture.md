...
- `.github/`  
    ...
    - `audit_check.yml` - Reusable, called by `entry.yml` after compile passes. Builds `wfx` once, then runs the three test audits (`base`, `endpoint`, `tls`) as parallel matrix jobs via `tests/run_audits.sh`.
