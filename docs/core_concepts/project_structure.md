...
<your_workspace>/
├─ <your_project_name>/
│  ├─ build/          # appears after the first build/run
│  ├─ intermediate/   # appears after the first build/run
...
Contains all log output generated by the server. This directory (both subfolders) is created automatically on the first `wfx run`, regardless of the `[Logging] enable_file` setting in `wfx.toml`. Whether anything actually gets written into `default_logs/` depends on `enable_file`; `crash_logs/` is always used regardless, since crash dumps aren't gated by that setting.

It contains two subdirectories:

- **`default_logs/`**  
  holds runtime log files. Each worker process writes to its own log file (`worker-0.log`, `worker-1.log`, etc.), and the master process writes to `master.log`. These files rotate automatically based on the size and rotation settings configured in `wfx.toml`.

- **`crash_logs/`**  
  holds crash dump files written by the crash tracer whenever a worker dies unexpectedly due to a signal (segfault, abort, etc.). Each dump is timestamped and named after the worker that crashed. These files are useful for post-mortem debugging.

Both directories and their contents can be safely deleted at any time. They will be recreated on the next server startup.
