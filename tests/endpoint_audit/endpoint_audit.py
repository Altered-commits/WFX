...
#
# WFX HttpEndpoint audit harness.
#
...
class Mock:
    ...
    def start(self):
        ...
        for _ in range(50):
            if self.ping():
                _log("mock", _green("up on %s:%d" % (self.cfg.host, self.cfg.up_port)))
                return
            time.sleep(0.1)
    ...
    def coalesce_count(self):
        try: return int(self.get("/ctl/coalesce/count"))
    ...
    def idle_conns(self):
        ...
        try: return int(self.get("/ctl/idleconns"))
        except ValueError: return -1
    ...
    def wait_ready(self):
        ...
        while time.time() - t0 < self.cfg.ready_timeout:
            ...
            if raw and _status_of(raw) == 200:
                ...
                return
            time.sleep(0.3)
        raise RuntimeError("WFX never answered /health within %ds" % self.cfg.ready_timeout)
...
def drive(cfg, path, ep="default", method="GET", want=None, fwd=None, body=None, x_body=None, rtimeout=8.0):
    ...
    if want: headers["X-Want"] = want
    if fwd: headers["X-Fwd"] = fwd
    if x_body is not None: headers["X-Body"] = x_body
...
def raw_head(cfg, fwd=None, fwd2=None, fwd3=None, method="GET", x_body=None):
    ...
    if fwd:  headers["X-Fwd"]  = fwd
    if fwd2: headers["X-Fwd2"] = fwd2
    if fwd3: headers["X-Fwd3"] = fwd3
    if x_body is not None: headers["X-Body"] = x_body
...
def phase_framing(cfg, mock, results):
    ...
    check(b, "content-length zero",        is_ok(drive(cfg, "/empty"), 200, ""))
    check(b, "content-length 1000",        (lambda r: is_ok(r, 200) and r.get("bodylen") == 1000)(drive(cfg, "/cl/1000")))
    check(b, "chunked single",             is_ok(drive(cfg, "/chunked/1"), 200, "[c0]"))
    check(b, "chunked multi (5)",          is_ok(drive(cfg, "/chunked/5"), 200, "[c0][c1][c2][c3][c4]"))
    check(b, "chunked extension ignored",  is_ok(drive(cfg, "/chunked-ext"), 200, "abc"))
    check(b, "chunked trailer discarded",  is_ok(drive(cfg, "/chunked-trailer"), 200, "xy"))
    check(b, "close-delimited body",       is_ok(drive(cfg, "/close"), 200, "closebody"))
    check(b, "http/1.0 close-delimited",   is_ok(drive(cfg, "/http10"), 200, "ten"))
    check(b, "HEAD is bodyless",           is_ok(drive(cfg, "/evil/headbody", method="HEAD"), 200, "") or
                                            is_ok(drive(cfg, "/kacount", method="HEAD"), 200, ""))
    check(b, "1xx: 100 then 200",          is_ok(drive(cfg, "/continue"), 200, "after"))
    check(b, "1xx: 8 informational ok",    is_ok(drive(cfg, "/continue/8"), 200, "done"))
    check(b, "response header retrievable", (lambda r: is_ok(r, 200) and r.get("hdr") == "alpha")(drive(cfg, "/ok", want="X-Mark")))
    check(b, "header get case-insensitive", (lambda r: is_ok(r, 200) and r.get("hdr") == "alpha")(drive(cfg, "/ok", want="x-mark")))
    for code in (200, 201, 202, 301, 400, 404, 418, 500, 599, 999):
        check(b, "status passthrough %d" % code, is_ok(drive(cfg, "/status/%d" % code), code))
...
def phase_statusline(cfg, mock, results):
    ...
    for name, line in ok_lines:
        ...
        check(b, "line ok: %s" % name, is_ok(r))
    ...
    for name, line in bad_lines:
        ...
        if name == "cr only line":
            r = drive_staged(cfg, mock, (line + "\nContent-Length: 0\r\n\r\n").encode("latin-1"))
            check(b, "line edge: %s" % name, is_alive(r))
            continue
        r = drive_staged(cfg, mock, (line + "\r\n\r\n").encode("latin-1"))
...
def phase_headers(cfg, mock, results):
    ...
    check(b, "empty value ok",        is_ok(drive_staged(cfg, mock, with_hdr("X-Empty:")), 200, "hi"))
    check(b, "value OWS trimmed",     (lambda r: is_ok(r, 200))(drive_staged(cfg, mock, with_hdr("X-A:    v   "))))
    check(b, "dup CL equal ok",       is_ok(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\nContent-Length: 2\r\n\r\nhi"), 200, "hi"))
    check(b, "CL leading zeros",      is_ok(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\nContent-Length: 0002\r\n\r\nhi"), 200, "hi"))
    check(b, "Connection: close ok",  is_ok(drive_staged(cfg, mock, with_hdr("Connection: close")), 200, "hi"))
    check(b, "Connection keep,close", is_ok(drive_staged(cfg, mock, with_hdr("Connection: keep-alive, close")), 200, "hi"))
    check(b, "Connection Close case", is_ok(drive_staged(cfg, mock, with_hdr("Connection: Close")), 200, "hi"))
    check(b, "Connection close,keep", is_ok(drive_staged(cfg, mock, with_hdr("Connection: close, keep-alive")), 200, "hi"))
    check(b, "Connection tabs tokens",is_ok(drive_staged(cfg, mock, with_hdr("Connection: \tkeep-alive\t,\tclose\t")), 200, "hi"))
    check(b, "Connection closed!=close",is_ok(drive_staged(cfg, mock, with_hdr("Connection: closed")), 200, "hi"))
    check(b, "value many colons ok",  (lambda r: is_ok(r, 200))(drive_staged(cfg, mock, with_hdr("X-A: a:b:c:d"))))
    check(b, "value all-spaces empty", is_ok(drive_staged(cfg, mock, with_hdr("X-A:      ")), 200, "hi"))
    check(b, "value tab-trimmed",     is_ok(drive_staged(cfg, mock, with_hdr("X-A:\tv\t")), 200, "hi"))
    check(b, "dup CL triple equal",   is_ok(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\nContent-Length: 2\r\nContent-Length: 2\r\n\r\nhi"), 200, "hi"))
    check(b, "CL trailing OWS ok",    is_ok(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\nContent-Length: 2 \r\n\r\nhi"), 200, "hi"))
    check(b, "many benign headers",   is_ok(drive_staged(cfg, mock, ("HTTP/1.1 200 OK\r\n" + "".join("X-H%d: v%d\r\n" % (i, i) for i in range(30)) + "Content-Length: 2\r\n\r\nhi").encode("latin-1")), 200, "hi"))
    check(b, "dup TE chunked ok",     is_ok(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\nTransfer-Encoding: chunked\r\n\r\n3\r\nabc\r\n0\r\n\r\n"), 200, "abc"))
    ...
    for name, blob in bad:
        ...
        sec = name in ("obs-fold continuation", "space before colon", "CL then TE", "TE then CL",
                       ...
                       "blank-ish fold line")
        check(b, "hdr bad: %s" % name, is_err(r), "expected err, got %r" % r, security=sec)
...
def phase_chunked(cfg, mock, results):
    ...
    for name, chunks, expect in ok:
        check(b, "chunk ok: %s" % name, is_ok(drive_staged(cfg, mock, CHUNKED(chunks)), 200, expect))

    # 200 tiny 1-byte chunks must reassemble to exactly 200 bytes.
    many = b"".join(b"1\r\n%c\r\n" % (0x61 + (i % 26)) for i in range(200)) + b"0\r\n\r\n"
    check(b, "chunk 200x1-byte reassembly", (lambda r: is_ok(r, 200) and r.get("bodylen") == 200)(
    ...
    for name, chunks in bad:
        ...
        check(b, "chunk bad: %s" % name, is_err(drive_staged(cfg, mock, CHUNKED(chunks),
...
def phase_eof(cfg, mock, results):
    ...
    for name, blob in trunc:
        check(b, "eof err: %s" % name, is_err(drive_staged(cfg, mock, blob, keep=False)), "expected err")

    # Legit EOF-delimited successes
    check(b, "eof close-delimited ok", is_ok(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\n\r\nbodybytes", keep=False), 200, "bodybytes"))
    check(b, "eof empty body ok",      is_ok(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n", keep=False), 200, ""))
    check(b, "zero-byte response",     is_err(drive(cfg, "/drop")))
    check(b, "connection reset (RST)", is_err(drive(cfg, "/reset")))
    # Huge declared CL, tiny body, then close -> reserve path then EOF error
    check(b, "huge CL then eof",       is_err(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\nContent-Length: 5000000\r\n\r\ntiny", keep=False)))
...
def phase_desync(cfg, mock, results):
    ...
    for name, poison in scenarios:
        ...
        check(b, "no poison after: %s" % name, clean,
    ...
    check(b, "trailing body not smuggled", (r is None) or (r.get("body") != "SMUGGLE"),
...
def phase_serialize(cfg, mock, results):
    ...
    for name, forged in [("Host lower", "host: evil.example"),
        ...
        check(b, "dedup %s" % name, ok, "reflected: %r" % (r and r.get("body")), security=True)
    ...
    check(b, "forward normal header", is_ok(r, 200) and "|xtest=hello" in (r.get("body") or ""))
    ...
    for n in (0, 1, 100, 1000):
        ...
        check(b, "CL correct POST %d" % n, ok, "reflected: %r" % (r and r.get("body")))

    # GET with no body: no Content-Length emitted at all.
    r = drive(cfg, "/reflect", method="GET")
    check(b, "GET emits no CL", is_ok(r, 200) and "|clen=-|" in (r.get("body") or ""))

    # Large forwarded header forces serialize() buffer-grow retry; must still arrive.
    big = "P" * 6000
    r = drive(cfg, "/reflect", fwd="X-Test: " + big)
    check(b, "buffer-grow big header", is_ok(r, 200) and ("|xtest=" + big) in (r.get("body") or ""))
    ...
    for name, payload in path_vectors:
        ...
        check(b, "reject path: %s" % name, is_errc(r, EP_SERIALIZE), "expected serialize-err, got %r" % r, security=True)
    ...
    for name, payload in hdr_vectors:
        ...
        check(b, "reject header: %s" % name, is_errc(r, EP_SERIALIZE), "expected serialize-err, got %r" % r, security=True)
    ...
    check(b, "space in path survives", is_alive(inject(cfg, "path", b"/a b")))

    # ── Byte-exact serialization via /reflectraw ('|' == CR/LF on the wire) ───
    hoststr = "%s:%d" % (cfg.host, cfg.up_port)
    h = raw_head(cfg)
    check(b, "raw GET request line", bool(h) and h.startswith("GET /reflectraw HTTP/1.1|"), "head=%r" % h)
    check(b, "raw Host exactly once", bool(h) and _count_ci(h, "|host:") == 1 and hoststr in h, "head=%r" % h)
    check(b, "raw GET emits no CL",  bool(h) and _count_ci(h, "content-length") == 0, "head=%r" % h)
    ...
    check(b, "raw forged Host dropped+order", ok, "head=%r" % h, security=True)
...
def phase_limits(cfg, mock, results):
    ...
    check(b, "header block under cap", is_ok(drive_staged(cfg, mock, hdr_block_bytes(200), ep="small")))
    check(b, "header block over cap",  is_err(drive_staged(cfg, mock, hdr_block_bytes(400), ep="small")))
    check(b, "single huge header line",is_err(drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\nX-P: " + b"A" * 600 + b"\r\nContent-Length: 0\r\n\r\n", ep="small")))

    def n_headers(n):
        block = "".join("X-H%d: v\r\n" % i for i in range(n))
        return ("HTTP/1.1 200 OK\r\n%sContent-Length: 0\r\n\r\n" % block).encode("latin-1")

    check(b, "header count at cap (8)", is_ok(drive_staged(cfg, mock, n_headers(7), ep="small")))
    check(b, "header count 8th ok",     is_ok(drive_staged(cfg, mock, n_headers(7), ep="small")))  # 7 + CL == 8
    check(b, "header count 9th over",   is_err(drive_staged(cfg, mock, n_headers(8), ep="small")))  # 8 + CL == 9
    check(b, "header count over cap",   is_err(drive_staged(cfg, mock, n_headers(20), ep="small")))

    # Exact byte boundary on maxBodyBytes for close-delimited + chunked framings.
    check(b, "chunk single at cap 1024",  (lambda r: is_ok(r, 200) and r.get("bodylen") == 1024)(
        drive_staged(cfg, mock, CHUNKED(b"400\r\n" + b"B" * 0x400 + b"\r\n0\r\n\r\n"), ep="small")))
    check(b, "chunk single 1025 over",    is_err(drive_staged(cfg, mock, CHUNKED(b"401\r\n" + b"B" * 0x401 + b"\r\n0\r\n\r\n"), ep="small")))
    check(b, "close body at cap 1024",    (lambda r: is_ok(r, 200) and r.get("bodylen") == 1024)(
        drive_staged(cfg, mock, b"HTTP/1.1 200 OK\r\n\r\n" + b"B" * 1024, keep=False, ep="small")))
    check(b, "header block exactly 256",  is_alive(drive_staged(cfg, mock, hdr_block_bytes(256), ep="small")))

    check(b, "body CL at cap (1024)",   (lambda r: is_ok(r, 200) and r.get("bodylen") == 1024)(
        drive_staged(cfg, mock, R("HTTP/1.1 200 OK", ["Content-Length: 1024"], b"B" * 1024), ep="small")))
    check(b, "body CL over cap",        is_err(drive_staged(cfg, mock, R("HTTP/1.1 200 OK", ["Content-Length: 1025"], b"B" * 1025), ep="small")))

    check(b, "chunk cumulative over cap", is_err(drive_staged(cfg, mock, CHUNKED(b"320\r\n" + b"B" * 0x320 + b"\r\n320\r\n" + b"B" * 0x320 + b"\r\n0\r\n\r\n"), ep="small")))
    ...
    check(b, "reason phrase over cap", is_err(drive_staged(cfg, mock,
...
def phase_resource(cfg, mock, results):
    ...
    t0 = time.time()
    check(b, "slow headers -> timeout", is_errc(drive(cfg, "/slow-headers", ep="fast", rtimeout=18), EP_REQ_TIMEOUT), "elapsed %.1fs" % (time.time() - t0))
    check(b, "slow body -> timeout",    is_errc(drive(cfg, "/slow-body", ep="fast", rtimeout=18), EP_REQ_TIMEOUT))
    ...
    check(b, "pool exhaustion resolves", all_resolved and none_ok, "results: %r" % res)
    ...
    check(b, "coalesce: 16 waiters all ok", all_ok, "results: %r" % res)
    check(b, "coalesce: backend hit once", hits == 1, "backend hits = %d (expected 1)" % hits)

    # Control: without coalescing, all 16 reach the backend.
    mock.coalesce_reset()
    _concurrent(lambda i: drive(cfg, "/coalesce", ep="default"), 16)
    hits = mock.coalesce_count()
    check(b, "no-coalesce: 16 backend hits", hits == 16, "backend hits = %d (expected 16)" % hits)
    ...
    check(b, "coalesce clone integrity", good, "a waiter got a truncated/aliased body")
...
def phase_fragmentation(cfg, mock, results):
    ...
    check(b, "drip CL body",        is_ok(drive_drip(cfg, mock, cl("hello"), piece=1), 200, "hello"))
    check(b, "drip empty body",     is_ok(drive_drip(cfg, mock, cl(""), piece=1), 200, ""))
    check(b, "drip 2-byte pieces",  is_ok(drive_drip(cfg, mock, cl("abcdefgh"), piece=2), 200, "abcdefgh"))
    check(b, "drip chunked",        is_ok(drive_drip(cfg, mock, CHUNKED(b"3\r\nabc\r\n2\r\nde\r\n0\r\n\r\n"), piece=1), 200, "abcde"))
    check(b, "drip chunked ext+tr", is_ok(drive_drip(cfg, mock, CHUNKED(b"3;x=y\r\nabc\r\n0\r\nX-T: v\r\n\r\n"), piece=1), 200, "abc"))
    ...
    for off in range(1, len(blob)):
        if not is_ok(drive_split(cfg, mock, blob, off), 200, "SPLITBODY"):
            bad_off = off
            break
    check(b, "split CL at every offset", bad_off is None, "misframed when split at byte %r" % bad_off)
    ...
    for k in (0, 1, 3, 5, 8, 12):
        check(b, "split chunked @ +%d" % k, is_ok(drive_split(cfg, mock, ch, base + k), 200, "abcdefgh"))
...
def phase_methods(cfg, mock, results):
    ...
    for m in ("GET", "OPTIONS", "DELETE"):
        ...
        check(b, "%s request line" % m, bool(h) and h.startswith("%s /reflectraw HTTP/1.1|" % m), "head=%r" % h)
        check(b, "%s no CL (no body)" % m, bool(h) and _count_ci(h, "content-length") == 0, "head=%r" % h)

    for m in ("POST", "PUT", "PATCH"):
        h = raw_head(cfg, method=m, x_body="abcd")
        ok = bool(h) and h.startswith("%s /reflectraw HTTP/1.1|" % m) and "|Content-Length: 4|" in h
        check(b, "%s body + CL" % m, ok, "head=%r" % h)
    ...
    for m in ("GET", "OPTIONS", "DELETE", "POST", "PUT", "PATCH"):
        ...
        check(b, "%s round-trips" % m, is_ok(r, 200, "hello"), "got %r" % r)
...
def phase_security(cfg, mock, results):
    ...
    check(b, "reuse A intact",        is_ok(drive(cfg, "/raw/secA", ep="reuse"), 201, "AAAAA"))
    check(b, "reuse B intact",        is_ok(drive(cfg, "/raw/secB", ep="reuse"), 202, "BBB"))
    check(b, "reuse A->B->A no bleed", is_ok(drive(cfg, "/raw/secA", ep="reuse"), 201, "AAAAA"), security=True)
    check(b, "reuse no header bleed",  (lambda r: is_ok(r, 202) and (r.get("hdr") in (None, "")))(
    ...
    check(b, "trailer body correct",   is_ok(drive(cfg, "/raw/sectr"), 200, "hi"))
    check(b, "trailer secret hidden",  (lambda r: is_ok(r, 200) and (r.get("hdr") in (None, "")))(
        drive(cfg, "/raw/sectr", want="X-Trailer-Secret")), "trailer surfaced as header", security=True)
    check(b, "trailer cookie hidden",  (lambda r: is_ok(r, 200) and (r.get("hdr") in (None, "")))(
    ...
    mock.stage("secinfo", b"HTTP/1.1 103 Early Hints\r\nX-Info-Leak: secret\r\nLink: </s>\r\n\r\n"
                          b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\n\r\nhi", keep=True)
    check(b, "1xx final body ok",      is_ok(drive(cfg, "/raw/secinfo"), 200, "hi"))
    check(b, "1xx header not leaked",  (lambda r: is_ok(r, 200) and (r.get("hdr") in (None, "")))(
    ...
    check(b, "CL body exact, extra dropped", is_ok(r, 200, "good") and r.get("bodylen") == 4,
    ...
    check(b, "drip 204+body: no data smuggled", no_smuggle,
          "phantom 204 body surfaced as a response", security=True)
    check(b, "drip 204+body: pool recovers", recovered,
    ...
    for name, p in path_ctrl:
        check(b, "reject path: %s" % name, is_errc(inject(cfg, "path", p), EP_SERIALIZE),
              "expected serialize-err, got %r" % inject(cfg, "path", p), security=True)
    ...
    for name, p in hdr_ctrl:
        check(b, "reject header: %s" % name, is_errc(inject(cfg, "header", p), EP_SERIALIZE),
    ...
    check(b, "no-newline flood capped", is_err(drive_drip(cfg, mock, b"H" * 4096, piece=64, ep="small", keep=False)))
    # Header-count amplification well beyond the cap.
    flood = ("HTTP/1.1 200 OK\r\n" + "".join("X-%d: v\r\n" % i for i in range(500)) + "Content-Length: 0\r\n\r\n").encode("latin-1")
    check(b, "header-count amplification capped", is_err(drive_staged(cfg, mock, flood, ep="small")))
...
def phase_lifecycle(cfg, mock, results):
    b = results.phase("lifecycle")
    ...
    dt = time.time() - t0
    check(b, "connect refused -> error", is_err(r) and dt < 16, "elapsed %.1fs r=%r" % (dt, r))
    ...
    dt = time.time() - t0
    check(b, "connect unreachable -> error", is_err(r) and dt < 22, "elapsed %.1fs r=%r" % (dt, r))

    # The worker must remain healthy after those connect failures.
    check(b, "worker survives connect fails", _wfx_healthy(cfg) and mock.ping())
    ...
    check(b, "close then reconnect ok",
    ...
    check(b, "keep-alive reuses conn", is_ok(r1, 200, "1") and is_ok(r2, 200, "2"),
          "r1=%r r2=%r" % (r1, r2))
    ...
    check(b, "idle timeout recycles conn", is_ok(r3, 200, "1"),
...
def report(results, server_alive):
    ...
    print(_bold("  TOTAL  %s   security-findings: %s   other-failures: %s" % (
        ...
        _red(str(sec_fail)) if sec_fail else _green("0"),
        _red(str(fail)) if fail else _green("0"))))
    if not server_alive: return 1
    if sec_fail: return 2
    if fail: return 1
    return 0
...
def main():
    ...
    try:
        ...
        # Prewarm evidence: EpPrewarm opens `prewarm` connections eagerly at boot.
        # They are accepted by the mock but send no request, so (accepted conns) -
        # (served requests) counts idle prewarmed connections. Snapshot it BEFORE
        # driving any request, polling briefly for the eager connects to land.
        cfg.prewarm_idle = -1
        for _ in range(50):   # poll up to ~5s for the eager connects to land
            cfg.prewarm_idle = mock.idle_conns()   # atomic conns-minus-requests
            if cfg.prewarm_idle >= 3:
                break
            time.sleep(0.1)
        _log("harness", "idle prewarmed connections at boot: %d" % cfg.prewarm_idle)
